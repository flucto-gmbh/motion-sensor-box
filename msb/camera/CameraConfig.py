...
class CameraConfig(MSBConfig):
    ...
    def _create_video_dir(self):
        ...
        try:
            ...
            self.video_dir = os.path.join(
                os.path.join(os.environ["HOME"], "msb_data"), "cam"
            )
            self._create_video_dir()
    ...
    def _parse_cmdline_args(self):
        ...
        args.add_argument("--video-dir", type=str, help="directory to store videos in")
    ...
    def _cmdline_config_override(self):
        ...
        if self._cmdline_conf["rollover_period"]:
            if self._cmdline_conf["verbose"] or self.verbose:
                print(
                    f"overriding rollover period setting with command line flag: {self._cmdline['rollover_period']}"
                )
        ...
        if self._cmdline_conf["video_dir"]:
            if self._cmdline_conf["verbose"] or self.verbose:
                print(
                    f"overriding video dir setting with command line flag: {self._cmdline['video_dir']}"
                )
