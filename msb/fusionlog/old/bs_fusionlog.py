...
try:
    from interval_logger import (
        calc_interval_from_timestamp,
        get_interval_file_handle,
        update_interval_file_handle,
    )
except Exception as e:
...
def main():
    ...
    while True:
        ...
        if id not in data_files:
            ...
            if not path.exists(data_files[id]["log_dir"]):
                try:
                    ...
                    print(
                        f'failed to create log file dir: {data_files[id]["log_dir"]}: {e}'
                    )
                    sys.exit(-1)

            data_files[id]["current_interval"] = calc_interval_from_timestamp(
                data[0], dt_interval=config["logfile_interval"]
            )
        ...
        if data[0] >= data_files[id]["current_interval"]:
            (
                data_files[id]["current_interval"],
                data_files[id]["file_handle"],
            ) = update_interval_file_handle(
