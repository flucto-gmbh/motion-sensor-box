...
class IMUConfig(MSBConfig):
    ...
    def _cmdline_config_override(self):
        ...
        if self._cmdline_conf["sample_rate_div"]:
            if self.verbose:
                print(
                    f"overriding output divisor with command line value {self._cmdline_conf['sample_rate_div']}"
                )
            self.sample_rate_div = self._cmdline_conf["sample_rate_div"]

        if self._cmdline_conf["acc_sensitivity"]:
            assert (
                cmdline_conf["acc_sensitivity"]
                in ICM20948_SETTINGS._acc_sensitivity_dict
            ), f"not a valid scale selection {cmdline_conf['acc_sensitivity']}"
            if self.verbose:
                print(
                    f"overriding acceleration sensitivity with command line value {self._cmdline_conf['acc_sensitivity']}"
                )
            self.acc_sensitivity = self._cmdline_conf["acc_sensitivity"]

        if self._cmdline_conf["gyr_sensitivity"]:
            assert (
                cmdline_conf["gyr_sensitivity"]
                in ICM20948_SETTINGS._gyr_sensitivity_dict
            ), f"not a valid scale selection {cmdline_conf['gyr_sensitivity']}"
            if self.verbose:
                print(
                    f"overriding gyroscope sensitivity with command line value {self._cmdline_conf['gyr_sensitivity']}"
                )
            self.gyr_sensitivity = self._cmdline_conf["gyr_sensitivity"]

        if self._cmdline_conf["acc_filter"]:
            assert self._cmdline_conf["acc_filter"] < len(
                ICM20948_SETTINGS._acc_filter_list
            ), f"not a valid filter selection {cmdline_conf['acc_filter']}"
            if self.verbose:
                print(
                    f"overriding acceleration filter setting with command line value {self._cmdline_conf['acc_filter']}"
                )
            self.acc_filter = self._cmdline_conf["acc_filter"]

        if self._cmdline_conf["gyr_filter"]:
            assert self._cmdline_conf["gyr_filter"] < len(
                ICM20948_SETTINGS._gyr_filter_list
            ), f"not a valid filter selection {cmdline_conf['gyr_filter']}"
            if self.verbose:
                print(
                    f"overriding gyroscope filter setting with command line value {self._cmdline_conf['gyr_filter']}"
                )
