...
def setup_socket(config: dict):
    ...
    try:
        ...
        logging.fatal(
            f'failed to bind to socket: {config["udp_address"], config["udp_port"]}'
        )
        sys.exit(-1)
    return s
...
def main():
    ...
    logging.debug(
        f'sample rate set to {config["sample_rate"]}, sleeping for {dt_sleep} s'
    )
    ...
    try:
        while True:
            ...
            # s.send_pyobj(data)
            s.send_multipart(
                [IMU_TOPIC, pickle.dumps(data)]  # topic  # serialize the payload
            )
