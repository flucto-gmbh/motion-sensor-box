...
class PileTrackConfig(MSBConfig):
    ...
    def _parse_cmdline_args(self):
        ...
        args.add_argument("--width", type=int, help="width of video")

        args.add_argument("--height", type=int, help="height of video")

        args.add_argument(
            "--rotate", action="store_true", help="rotate video 90 deg counterclockwise"
        )
        ...
        args.add_argument(
            "--px-to-m", type=float, help="conversion ratio from pixels to meters"
        )
    ...
    def _cmdline_config_override(self):
        ...
        if self._cmdline_conf["show_video"]:
            self._print_verbose(
                "overriding show video flag using command line input. WARNING: requires a running desktop environment"
            )
        ...
        if self._cmdline_conf["fps"]:
            self._print_verbose(
                f"overriding fps setting with command line flag: {self._cmdline_conf['fps']}"
            )
            self.fps = self._cmdline_conf["fps"]

        if self._cmdline_conf["width"]:
            self._print_verbose(
                f"overriding width setting with command line flag: {self._cmdline['width']}"
            )
            self.width = self._cmdline_conf["width"]

        if self._cmdline_conf["height"]:
            self._print_verbose(
                f"overriding height setting with command line flag: {self._cmdline['height']}"
            )
            self.height = self._cmdline_conf["height"]

        if self._cmdline_conf["rotate"]:
            self._print_verbose(
                f"overriding rotate flag with command line flag: rotating video by 90 deg counterclockwise"
            )
            self.rotate = True
        ...
        if self._cmdline_conf["region_of_interest"]:
            self._print_verbose(
                "overrding region of interest via command line: {self._cmdline_conf['region_of_interest']}"
            )
            self.region_of_interest = self._parse_pile_tracking_roi()

        if self._cmdline_conf["max_number_of_features"]:
            self._print_verbose(
                f"overriding max. number of features via command line. Now tracking {self._cmdline_conf['max_number_of_features']} features"
            )
            self.number_of_features = self._cmdline_conf["max_number_of_features"]

        if self._cmdline_conf["max_track_length"]:
            self._print_verbose(
                f"overriding max track length parameter using command line. New max track length is {self._cmdline_conf['max_track_length']}"
            )
            self.max_track_length = self._cmdline_conf["max_track_length"]

        if self._cmdline_conf["pile_speed_update_interval"]:
            self._print_verbose(
                f"overriding pile speed calculation interval via command line. Calculating pile speed every {self._cmdline_conf['pile_speed_update_interval']} frames"
            )
            self.pile_speed_calculation_interval = self._cmdline_conf[
                "pile_speed_calculation_interval"
            ]

        if self._cmdline_conf["px_to_m"]:
            self._print_verbose(
                f"overriding pixels to meter ratio via command line. Pixels to meters ratio is now {self._cmdline_conf['px_to_m']} frames"
            )
        ...
        if self._cmdline_conf["meter_markings_score_threshold"]:
            self._print_verbose(
                f"overriding meter markings score threshold via command line. Meter marking score threshold is now {self._cmdline_conf['meter_markings_score_threshold']}"
            )
            self.meter_markings_score_threshold = self._cmdline_conf[
                "meter_markings_score_threshold"
            ]

        if self._cmdline_conf["meter_markings_region_of_interest"]:
            self._print_verbose(
                f"overriding meter markings region of interest via command line. Meter markings region of interest is now {self._cmdline_conf['meter_markings_region_of_interest']}"
            )
        ...
        if self._cmdline_conf["meter_markings_update_interval"]:
            self._print_verbose(
                f"overriding meter markings calculation interval via command line. Now calculating meter marking position every {self._construct_zmq_socketstrings['meter_markings_calculation_interval']} frames"
            )
            self.meter_markings_calculation_interval = self._cmdline_conf[
                "meter_markings_calculation_interval"
            ]
