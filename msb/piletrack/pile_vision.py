...
def find_meter_digits(
    img_gray,
    number_templates: list,
    threshold=0.5,
    expected_meter_digit=None,
    expected_ypos=[None, None],
    expected_digit_boost_factor=1.1,
    do_write_as_png=False,
) -> dict:
    ...
    for template, number in number_templates:
        ...
        if number == expected_meter_digit:
            ...
            if maxLoc[1] > expected_ypos[0] and maxLoc[1] < expected_ypos[1]:
                print(
                    f"Number {number} with maxVal={maxVal} is in y-range and gets expected_digit_boost_factor boost."
                )
        ...
        if maxVal > highest_maxVal:
            ...
            loc = np.where(
                res == highest_maxVal_unboosted
            )  # Find best matching template.
            if (
                len(loc[0]) > 0 and maxVal > threshold
            ):  # Check wheter it matches better than the threshold.
                if do_write_as_png:
                    for pt in zip(*loc[::-1]):
                        ...
                        cv.rectangle(
                            img_rgb,
                            pt,
                            (pt[0] + template_w, pt[1] + template_h),
                            (0, 0, 255),
                            2,
                        )
                ...
                if len(loc[0]) > 1:
                    warnings.warn(
                        f"len(loc[0]) = {len(loc[0])}. Multiple locations with the same goodness score for template matching have been found. Continuing with the location with index=0."
                    )
    ...
    if x is not None:
        res = {
            "found_m_number": found_m_number,
            "number_xy": (x, y),
            "number_wh": (template_w, template_h),
            "goodness": highest_maxVal_unboosted,
        }
...
if __name__ == "__main__":
    ...
    img_rgb = cv.imread(
        "figs/video_5-36-56(2)/Aft Door Camera_urn-uuid-00306F00-0030-6F4A-155B-00306F4A155B_2022-07-09_05-36-56(2).mp4_42000.jpg"
    )
