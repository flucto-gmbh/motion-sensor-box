...
class RegionOfInterest:
    ...
    roi_type: str = (
        "pile"  # Type of ROI, can be "pile" or "marking_number". Defaults to "pile".
    )
    ...
    def __post_init__(self):
        ...
        if self.roi_type == "pile":
            if self.cam == "osbit_aft_door":
                ...
                self.roi_up_lim = int(
                    0.021 * self.height
                )  # pixel coordinates, 0 = very top of image
                ...
                self.roi_right_lim = int(
                    0.565 * self.width
                )  # When small crane obstructs UET 0.53, otherwise 0.55 is fine
                self.roi_up_lim = int(
                    0.25 * self.height
                )  # Could be as low as 0.01, in video 0082 set to 0.15, in video 0084 to 0.10, in video 0086 0.2
...
class CameraInfo:
    ...
    roller_line: np.array = np.array(
        [[140, 635], [580, 800], [1100, 710], [1237, 540]], np.int32
    )
...
class Site:
    name: str = "YUN56"
    gripper_keel_dist = (
        27.132  # for DLS 4200 during Yunlin installation, symbol g_v in drawing
    )
...
def track_video(
    ...
    if frame is None:
        raise ValueError(
            f"frame is None. Possibly the start_frame_idx of {start_frame_idx} is too high (higher than the total frame count)."
        )
    pile_mask = np.zeros(frame.shape[:2], dtype="uint8")
    cv.rectangle(
        pile_mask,
        (roi_pile.roi_left_lim, roi_pile.roi_up_lim),
        (roi_pile.roi_right_lim, roi_pile.roi_down_lim),
        255,
        -1,
    )
    ...
    cv.rectangle(
        numbers_mask,
        (roi_numbers.roi_left_lim, roi_numbers.roi_up_lim),
        (roi_numbers.roi_right_lim, roi_numbers.roi_down_lim),
        255,
        -1,
    )
    ...
    if do_export_video:
        ...
        out = cv.VideoWriter(
            output_folder + "tracking_" + fname, fourcc, 20.0, (fwidth, fheight)
        )
    ...
    while True:
        ...
        if len(tracks) > 0:
            ...
            px_to_m = px_to_m_conversion_factor(
                all_x, fname=fname, do_plot_calibration_curve=False
            )
            # print("px_to_m: " + str(px_to_m))
            vels_mps = px_to_m * vels * fps
            median_vel_mps[frame_idx] = np.median(vels_mps)
            travelled_distance[frame_idx] = (
                travelled_distance[frame_idx - 1] + median_vel_mps[frame_idx] / fps
            )

            vert_distance = 20
            drawing_line = 1
            draw_str(
                vis, (20, drawing_line * vert_distance), "frame id: " + str(frame_idx)
            )
            drawing_line += 1
            draw_str(
                vis, (20, drawing_line * vert_distance), "track count: %d" % len(tracks)
            )
            drawing_line += 1
            vel_last_1s = np.mean(median_vel[frame_idx - fps : frame_idx])
            draw_str(
                vis,
                (20, drawing_line * vert_distance),
                "mean velocity last 1s (px/frame): {:.2f}".format(vel_last_1s),
            )
            drawing_line += 1
            vel_last_1s_mmpermin = (
                np.mean(median_vel_mps[frame_idx - fps : frame_idx]) * 1000 * 60
            )
            draw_str(
                vis,
                (20, drawing_line * vert_distance),
                "mean velocity last 1 s (mm/min): {:.0f}".format(vel_last_1s_mmpermin),
            )
            drawing_line += 1
            t_average = 5  # 5 seconds
            vel_last_5s = np.mean(
                median_vel_mps[frame_idx - t_average * fps : frame_idx]
            )
            vel_last_5s = vel_last_5s * 1000 * 60
            draw_str(
                vis,
                (20, drawing_line * vert_distance),
                "mean velocity last 5 s (mm/min): {:.0f}".format(vel_last_5s),
            )
            drawing_line += 1
            t_average = 30
            vel_last_long_s = np.mean(
                median_vel_mps[frame_idx - t_average * fps : frame_idx]
            )
            ...
            draw_str(
                ...
                "mean velocity last "
                + str(t_average)
                + " s (mm/min): {:.0f}".format(vel_last_long_s),
            )
            drawing_line += 1
            ...
            draw_str(
                vis,
                (20, drawing_line * vert_distance),
                "penetration depth (m): {:.3f}".format(penetration_depth),
            )
            drawing_line += 1

            draw_str(
                vis,
                (20, int(0.95 * frame.shape[0])),
                "'d' to delete features | 'ESC' to stop program",
            )
        alltracks_vel.append(vels)

        # Every x frames we look for new speed features (and if we have less than we would like to have).
        if (
            frame_idx % detect_speed_interval == 0
            and len(tracks) < feature_params["maxCorners"]
        ):
            mask = pile_mask
        ...
        if frame_idx % detect_markers_interval == 0:
            ...
            frame_roi_numbers = frame_gray[
                r.roi_up_lim : r.roi_down_lim, r.roi_left_lim : r.roi_right_lim
            ]
            if do_search_m_marking_numbers:
                d = find_meter_digits(
                    frame_roi_numbers,
                    number_templates,
                    threshold=meter_digit_threshold,
                    expected_meter_digit=expected_meter_digit,
                    expected_ypos=expected_ypos,
                    do_write_as_png=True,
                )
                print(d)
                if d is not None:
                    ...
                    number_xy = (
                        number_xy[0] + r.roi_left_lim,
                        number_xy[1] + r.roi_up_lim,
                    )
                    ...
                    if (
                        len(found_numbers) >= n
                        and len(set(found_numbers[-n:])) == 1
                        and np.isnan(found_numbers[-1]) == False
                    ):
                        ...
                        if expected_ypos[0] < 0:
                            expected_ypos[0] = 0
                        if expected_ypos[1] > 1:
                            expected_ypos[1] = 1
                    else:
                        is_stable_number = False
                        expected_meter_digit = None
        ...
        if found_m_number is not None:
            n_px_is_1m_at_center = int(
                1
                / px_to_m_conversion_factor(
                    p_center, fname=fname, do_plot_calibration_curve=False
                )
            )
            n_px_is_1m_at_m_numbers = int(
                1
                / px_to_m_conversion_factor(
                    number_xy[0], fname=fname, do_plot_calibration_curve=False
                )
            )
            travelled_since_last_detection = (
                travelled_distance[frame_idx] - travelled_at_last_detection
            )
            ...
            new_number_xy = (
                number_xy[0],
                number_xy[1]
                + int(travelled_since_last_detection * n_px_is_1m_at_m_numbers),
            )
            ...
            text_xy = (
                number_xy[0] + number_wh[0] + 1,
                number_xy[1] + int(0.5 * number_wh[1]),
            )
            cv.putText(
                vis,
                str(found_m_number),
            ...
            cv.putText(
                vis,
                str(found_m_number),
                (p_center - 10, text_xy[1]),
                fontFace=cv.FONT_HERSHEY_PLAIN,
                fontScale=1.0,
                color=m_marker_color_bgr,
                thickness=1,
                lineType=cv.LINE_AA,
            )

            # Estimate monopile height at gripper position
            px_dist_to_found_number = (
                cam_info.roller_y_at_m_marking_numbers - number_xy[1]
            )
            m_dist = px_dist_to_found_number * px_to_m_conversion_factor(
                p_center, fname=fname, do_plot_calibration_curve=False
            )
            ...
            pt2 = (
                p_center + m_marker_line_length + offset,
                cam_info.roller_y_at_p_center,
            )
            cv.line(vis, pt1, pt2, m_marker_color_bgr, 1)
            cv.putText(
                vis,
                f"{marker_at_gripper_height:.3f}",
                (pt1[0] - 10, pt1[1]),
                fontFace=cv.FONT_HERSHEY_PLAIN,
                fontScale=1.0,
                color=m_marker_color_bgr,
...
roi_pile = RegionOfInterest(
    roi_type="pile", cam=cam, width=resolution[1], height=resolution[0]
)  # Default ROI for aft cam
roi_marking_numbers = RegionOfInterest(
    roi_type="marking_numbers", cam=cam, width=resolution[1], height=resolution[0]
)
print(f"roi_marking_numbers: {roi_marking_numbers}")
cam_info = CameraInfo(
    name=cam,
    roi_pile=roi_pile,
    roi_meter_marking_numbers=roi_marking_numbers,
...
(
    ...
    fpath=fpath,
    cam_info=cam_info,
    do_export_video=do_export_video,
    track_max_len=10,
    detect_speed_interval=5,
    detect_markers_interval=10,
)
...
if tracking_bad_start_id is not None:
    ...
    ax.add_patch(
        Rectangle(
            (tracking_bad_start_id, np.nanmin(median_vel)),
            width,
            height,
            facecolor="r",
            edgecolor="none",
    ...
    ax.text(
        tracking_bad_start_id,
        np.nanmax(median_vel),
        "Algorithm bad from here",
        fontsize=6,
        color="r",
...
fig_folder = "figs/"
fig1.savefig(
    fig_folder + "plot_" + fname[:-4] + "pixelperframe.png", dpi=300, format="png"
)
...
abs_times = pd.date_range(
    start_date, periods=len(frame_ids), freq=str(pandas_freq) + "ms"
)
...
fig2.savefig(
    fig_folder + "plot_" + fname[:-4] + "_physicalunits.png", dpi=300, format="png"
)
