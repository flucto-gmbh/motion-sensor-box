...
class TestSceneRender:
    ...
    def getRectInTime(self, time):
        if self.foreground is not None:
            tmp = np.array(self.center) + np.array(
                (self.getXOffset(time), self.getYOffset(time))
            )
            x0, y0 = tmp
            x1, y1 = tmp + self.foreground.shape[0:2]
            return np.array([y0, x0, y1, x1])
        else:
            x0, y0 = self.initialRect[0] + np.array(
                (self.getXOffset(time), self.getYOffset(time))
            )
            x1, y1 = self.initialRect[2] + np.array(
                (self.getXOffset(time), self.getYOffset(time))
            )
            return np.array([y0, x0, y1, x1])
    ...
    def getNextFrame(self):
        ...
        if self.foreground is not None:
            ...
            img[
                self.currentCenter[0] : self.currentCenter[0]
                + self.foreground.shape[0],
                self.currentCenter[1] : self.currentCenter[1]
...
class Cube(VideoSynthBase):
    def __init__(self, **kw):
        ...
        self.render = TestSceneRender(
            cv.imread(cv.samples.findFile("pca_test1.jpg")), deformation=True, speed=1
        )
...
class Chess(VideoSynthBase):
    ...
    def draw_quads(self, img, quads, color=(0, 255, 0)):
        img_quads = cv.projectPoints(
            quads.reshape(-1, 3), self.rvec, self.tvec, self.K, self.dist_coef
        )[0]
