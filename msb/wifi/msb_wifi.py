    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        for topic, data in get_data_zmqxpub(zmq_sub_socket):
            ...
            try:
                udp_socket.sendto(
                    pickle.dumps(
                        {"msb-sn": wifi_config.serialnumber, "payload": {topic: data}}
                    ),
                    (
                        wifi_config.targets[target]["target_address"],
                        wifi_config.targets[target]["target_port"],
                    ),
                )
            except Exception as e:
                if wifi_config.verbose:
                    print(
                        f"""failed to send data for target {target} \
{wifi_config.targets[target]['target_address']}:\
...
def msb_wifi(wifi_config: WifiConfig):
    ...
    for target, _ in wifi_config.targets.items():
        ...
        wifi_processes[target] = Process(
            target=consume_send_msbdata,
            args=(
                target,
                wifi_config,
            ),
