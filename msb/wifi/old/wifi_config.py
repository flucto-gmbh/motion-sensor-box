...
def dump_config_file(config: dict):
    with open(config["dump_config_file"], "w") as config_file_handle:
        config_file_handle.writelines(json.dumps(config, indent=4))
...
def parse_arguments() -> dict:
    ...
    arg_parser.add_argument(
        "--print", action="store_true", help="prints the incoming data", default=False
    )

    arg_parser.add_argument("--logfile", help="path to logfile", type=str, default=None)
    ...
    arg_parser.add_argument(
        "--udp-port", help="port to stream darta to", default=9870, type=int
    )

    arg_parser.add_argument(
        "--ipc-port-pubx", help="IPC port used by zeroMQ", default=5556, type=int
    )

    arg_parser.add_argument(
        "--ipc-port-subx", help="IPC port used by zeroMQ", default=5555, type=int
    )
...
def init() -> dict:
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler(config["logfile"])]
        if config["logfile"]
