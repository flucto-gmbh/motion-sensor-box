...
from msb.network.types import ZMQ_Publisher, ZMQ_Subscriber
from msb.network.mqtt.config import ZMQConf
...
def test_subscriber_without_broker():
    ...
    pub.bind(f"tcp://*:{port}")
    subscriber_config = ZMQConf(subscriber_port=port)
    sub = ZMQ_Subscriber(test_topic, subscriber_config)
    wait()
